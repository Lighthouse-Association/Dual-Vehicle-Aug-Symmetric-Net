...
import torch
import time
from ..benchmark import get_mrr, accuracy
...
def gnn_rerank_params_grid_search(all_lang_embeds, all_visual_embeds):
    ...
    for k1, k2, lamb in itertools.product(k1_params, k2_params, lambda_params):
        ...
        if mrr_.item() * 100 > best_mrr:
            ...
            rerank_acc5, rerank_acc10 = accuracy(sim_rerank, torch.arange(sim_rerank.size(0)).to(sim_rerank.device),
                                                topk=(5, 10))
            print(
                f"better_mrr = {best_mrr}, k1 = {k1}, k2 = {k2}, eps = {lamb}, acc5 = {rerank_acc5}, acc10 = {rerank_acc10}")
...
def rerank_params_grid_search(all_lang_embeds, all_visual_embeds, rank_type='k-rec'):
    ...
    for k1, k2, lamb in itertools.product(k1_params, k2_params, lambda_params):
        ...
        if mrr_.item()*100 > best_mrr:
            ...
            rerank_acc5, rerank_acc10 = accuracy(sim_rerank, torch.arange(sim_rerank.size(0)).to(sim_rerank.device),
                                                topk=(5, 10))
            print(f"better_mrr = {best_mrr}, k1 = {k1}, k2 = {k2}, eps = {lamb}, acc1 = {rerank_acc5}, acc5 = {rerank_acc10}")
    return best_mrr, best_params, best_sim_rerank
