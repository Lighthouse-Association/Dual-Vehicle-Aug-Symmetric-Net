...
from .utils import prepare_start
from .test import inference_vis_and_lang


# with open("data2022/test-queries_nlpaug.json") as f:
with open("/datasets/aicitychallenge2023/test-queries.json") as f:
    queries = json.load(f)
with open("/datasets/aicitychallenge2023/test-tracks.json") as f:
    tracks = json.load(f)
...
def feature_mean_score_test(args, config_list, merge_weights, enforced=False, spatial=False):
    print("FEAT_ENG:", args.cfg.DATA.FEAT_ENG)
    img_feats = []
    nlp_feats = []
...
def main():
    ...
    feat = -1
    args.save_name = f'ensemble_spatial_w.json'
