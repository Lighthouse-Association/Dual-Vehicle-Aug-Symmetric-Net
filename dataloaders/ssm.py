...
class CityFlowNLDatasetSSM(Dataset):
...
class CityFlowNLInferenceDatasetSSM(Dataset):
...
class CityFlowNLDatasetValSSM(Dataset):
    def __init__(self, data_cfg, json_path, transform=None, Random=True, motion_transform=None, years=2022):
        """
        Dataset for training.
        :param data_cfg: CfgNode for CityFlow NL.
...
def make_data_loader(cfg, transform=None, motion_transform=None, val_transform=None, is_train=True):
    if is_train:
        train_data=CityFlowNLDatasetSSM(cfg.DATA, json_path=cfg.DATA.TRAIN_JSON_PATH,
        ...
        val_data_test = CityFlowNLInferenceDatasetSSM(cfg.DATA, transform=val_transform, val=True)
