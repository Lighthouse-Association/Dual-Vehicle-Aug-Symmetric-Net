...
class CityFlowNLDatasetLAggr(Dataset):
    def __init__(self, data_cfg,json_path,transform = None,Random= True, motion_transform=None, years=2022):
        ...
        print("FEAT_ENG:", self.data_cfg.FEAT_ENG)
    ...
    def __getitem__(self, index):
        ...
        lang_feats_dict = extract_feats_from_nl(self.data_cfg, track["nl"], track["nl_other_views"])
        ...
        if self.data_cfg.USE_FEATS_QUERY:
            text = lang_feats_dict["q"]
            car_text = text
            
        if self.data_cfg.USE_FEATS_AUG_TEXT:
            text = lang_feats_dict["aug_txt"] + text
            car_text = lang_feats_dict["aug_car"] + car_text + lang_feats_dict["aug_car_end"]
...
class CityFlowNLInferenceDatasetLAggr(Dataset):
...
class CityFlowNLDatasetValLAggr(Dataset):
    def __init__(self, data_cfg, json_path, transform=None, Random=True, motion_transform=None, years=2022):
        """
        Dataset for training.
        :param data_cfg: CfgNode for CityFlow NL.
...
def make_data_loader(cfg, transform=None, motion_transform=None, val_transform=None, is_train=True):
    if is_train:
        train_data=CityFlowNLDatasetLAggr(cfg.DATA, json_path=cfg.DATA.TRAIN_JSON_PATH,
                                 transform=transform, motion_transform=motion_transform)
        ...
        val_data_test = CityFlowNLInferenceDatasetLAggr(cfg.DATA, transform=val_transform, val=True)
        val_loader = DataLoader(dataset=val_data_test, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=False,
                                num_workers=cfg.TRAIN.NUM_WORKERS, pin_memory=True, )
        return train_data, train_loader, val_data_test, val_loader
    return None
