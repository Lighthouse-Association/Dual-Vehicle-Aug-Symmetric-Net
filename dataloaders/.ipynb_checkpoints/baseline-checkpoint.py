...
class CityFlowNLDatasetBaseline(Dataset):
...
class CityFlowNLInferenceDatasetBaseline(Dataset):
    def __init__(self, data_cfg):
...
def make_data_loader(cfg, is_train=True):
    if is_train:
        dataset = CityFlowNLDatasetBaseline(cfg.DATA)
        train_sampler = DistributedSampler(dataset)
        return dataset, DataLoader(dataset, batch_size=cfg.TRAIN.BATCH_SIZE,
                            num_workers=cfg.TRAIN.NUM_WORKERS,
                            sampler=train_sampler)
    dataset = CityFlowNLInferenceDatasetBaseline(cfg.DATA)
    return dataset, DataLoader(dataset, batch_size=cfg.EVAL.BATCH_SIZE, num_workers=cfg.EVAL.NUM_WORKERS)
